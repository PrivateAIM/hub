...
- **`displayName`** — an optional, free-form human-readable label. When set, it is shown in the UI in place of the `name`.

The opaque analysis id (UUID) is no longer surfaced in list views or page headlines — analyses are identified by their `displayName` (falling back to `name`).
