...
/**
 * Release-window consolidation of two unreleased migrations (see
 * conventions.md#database-migrations): aligns seven stale constraint names on
 * `analyses` (issue #1823), then adds the 58 single-column indexes backing
 * the rapiq query surface across all twelve server-core entities (issue
 * #1842). Squashed under the earlier timestamp because the second migration
 * depends on the first having already run.
 *
 * ## Part 1 — align seven stale `analyses` constraint names
 *
 * The table was created as `analysis_entity` (Default1771579910800), renamed
 * to `analysis` (AddDisplayName1780300000000) and finally to `analyses`
 ...
 * constraint names from table + column names — so the live schema still held
 * `analysis_entity`-derived hashes while the entity metadata derives
 * `analyses`-based ones. Every `migration generate` therefore opened with
 * seven spurious drop/recreate statements before any real change, and the
 * drift gate (`scripts/assert-schema-drift.mjs`) would report the same
 * noise. Aligning the names unblocks clean generate output and a quiet gate.
 *
 * The four indexes rename through RENAME INDEX. MySQL has no RENAME
 * CONSTRAINT, so each foreign key is dropped and re-added byte-identical
 * apart from the name — no row is read or written and no table is rebuilt:
 *
 ...
 *
 * ## Part 2 — indexes for the query surface
 *
 ...
 *
 * The `analyses` index names assume part 1's renames have already run in this
 * same `up()`, i.e. every constraint on that table already carries its
 ...
 */
export class AlignAnalysesConstraintNamesAndAddQueryIndexes1788400000000 implements MigrationInterface {
    name = 'AlignAnalysesConstraintNamesAndAddQueryIndexes1788400000000';

    public async up(queryRunner: QueryRunner): Promise<void> {
        // Part 1 — align stale `analyses` constraint names.
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        // Part 2 reversed first — hand-authored: indexes drop in reverse
        // creation order, except the eight serving a foreign key (InnoDB
        // dropped their constraints' implicit indexes when up() created
        // them). Those are RENAMED back to the constraint's own name —
        // RENAME INDEX is metadata-only and legal while the constraint is
        // live — which IS the pre-migration state: an implicit FK index
        // named after its constraint. The constraint itself is never
        // touched, so its cascades keep firing throughout and nothing is
        // ever re-added unvalidated.
        await queryRunner.query(`
        ...
        // Part 1 reversed — restore the seven stale constraint names.
        await queryRunner.query('SET FOREIGN_KEY_CHECKS = 0');
        try {
