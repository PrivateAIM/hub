...
export type DecodeQueryOptions<RECORD extends ObjectLiteral = ObjectLiteral> = {
     ...
     * that is not listed is neither parsed nor defaulted. E.g. a bulk
     * selection processes `['filters']` only, so a schema
     * `pagination.maxLimit` can never silently truncate the affected row set.
     */
