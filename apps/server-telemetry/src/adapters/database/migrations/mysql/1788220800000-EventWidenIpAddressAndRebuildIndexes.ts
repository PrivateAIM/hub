 ...
 *    Net: 14 indexes -> 7.
 *
 * Note `realm_id` is varchar(255) on MySQL (typeorm maps `uuid` -> varchar/255),
 * so `(realm_id, created_at)` is a ~1028-byte utf8mb4 key — under InnoDB's
 * 3072-byte limit, but deliberately not narrowed here: that is extra DDL with
 * no query win.
 *
 * `down()` narrows `request_ip_address` back to 15, which fails if an IPv6
 * value has been stored by then. Best-effort, and safe on the CI round-trip,
 * which reverts against an empty table.
 */
export class EventWidenIpAddressAndRebuildIndexes1788220800000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query('DROP INDEX `IDX_73da2faef487086e700845b7b8` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_d18c751252944d7287e01f2bba` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_27a4b4d027f84645cff6a2046b` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_98091bf7c210a05180c50b239d` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_96aea52eeb823dd8a3708802c0` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_8209335b1c8632c756ce649d44` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_ca050bac0a654d1e096beb5d46` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_d1bde4e07e02555eadc160c0de` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_8c05c0a6344a9ba945d9e7c9d2` ON `events`');

        await queryRunner.query('ALTER TABLE `events` MODIFY `request_ip_address` varchar(45) NULL');
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query('DROP INDEX `IDX_7ebab07668bb225b6a04782a7d` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_ae92f92677aad1f01f61bc7347` ON `events`');
        await queryRunner.query('DROP INDEX `IDX_d081c16020e8daee7dea5d3ca7` ON `events`');

        await queryRunner.query('ALTER TABLE `events` MODIFY `request_ip_address` varchar(15) NULL');
