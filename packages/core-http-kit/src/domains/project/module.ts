...
export class ProjectAPI extends BaseAPI {
    ...
    async getOne(id: Project['id'], requestRecord?: EntityQueryInput<Project>): Promise<EntityRecordResponse<Project>> {
        const response = await this.client.get(`projects/${id}${buildQueryString(requestRecord)}`);

        return response.data;
    }

    async create(data: ProjectCreatePayload): Promise<EntityRecordResponse<Project>> {
        const response = await this.client.post('projects', nullifyEmptyObjectProperties(data));

        return response.data;
    }

    async delete(id: Project['id']): Promise<EntityRecordResponse<Project>> {
        const response = await this.client.delete(`projects/${id}`);
        return response.data;
    }

    async update(id: Project['id'], data: ProjectUpdatePayload): Promise<EntityRecordResponse<Project>> {
